...
import { Package, Eye, Calendar, MapPin, Search, Filter, Loader2 } from 'lucide-react';
    ...
    return (
      <div className="container mx-auto px-16 py-8">
        <div className="text-center py-12">
          ...
          <Button onClick={() => router.push('/auth/login?redirect=/orders')}>
            Log In
          </Button>
        </div>
      </div>
    ...
    <div className="container mx-auto px-16 py-8">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12">
          ...
          {filters.search || filters.status !== 'all' ? (
            <Button onClick={() => setFilters({ search: '', status: 'all', page: 1 })}>
              Clear Filters
            </Button>
          ) : (
                ...
                <CardHeader>
                  <div className="flex justify-between items-start">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-lg font-bold mt-2">{Number(order.total).toLocaleString()} RWF</p>
                    </div>
                  </div>
                </CardHeader>
                <CardContent>
                    ...
                    {order.items && order.items.length > 0 ? (
                      order.items.slice(0, 3).map((item, index) => (
                        <div key={index} className="flex justify-between items-center py-2 border-b last:border-b-0">
                          ...
                          <p className="font-medium">{Number(item.total).toLocaleString()} RWF</p>
                        </div>
                      ))
                    ) : (
                      <div className="py-2 text-muted-foreground">No items found</div>
                    )}
          ...
          {ordersData && ordersData.count > 10 && (
            <div className="flex justify-center items-center space-x-2 mt-8">
              ...
              <span className="text-sm text-muted-foreground">
                Page {filters.page} of {Math.ceil(ordersData.count / 10)}
              </span>
              
              <Button
                variant="outline"
                onClick={() => handlePageChange(filters.page + 1)}
                disabled={filters.page >= Math.ceil(ordersData.count / 10)}
              >
                Next
              </Button>
            </div>
