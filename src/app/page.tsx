...
const FeaturedProducts = lazy(() => import('@/components/landing-page/FeaturedProducts'))
const MoreProducts = lazy(() => import('@/components/landing-page/MoreProducts'))
...
const page: FC<pageProps> = ({}) => {
  return <div className='w-full'>
    ...
    <NavBar />
    <HeroCarousel />
    <Suspense fallback={<div className="my-20 flex justify-center"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-brand-orange"></div></div>}>
      <Collection />
    </Suspense>
    <Suspense fallback={
      <div className="my-20">
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="grid grid-cols-2 min-[500px]:grid-cols-3 min-[1000px]:grid-cols-4 xl:grid-cols-4 gap-5">
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className="aspect-[9/12] bg-gray-200 rounded-2xl animate-pulse"></div>
            ))}
          </div>
        </div>
      </div>
