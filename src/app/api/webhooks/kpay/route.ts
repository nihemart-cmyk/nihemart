...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { data: payment, error: findError } = await supabase
      ...
      .or(`kpay_transaction_id.eq.${paymentStatus.transactionId},reference.eq.${paymentStatus.orderReference}`)
      .single();
    ...
    if (paymentStatus.isFailed) {
      ...
      if (orderUpdateError) {
        ...
        console.log('Order status updated to cancelled due to payment failure:', payment.order_id);
      }
    }
