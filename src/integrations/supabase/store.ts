...
const buildStoreQuery = ({ search = '', filters = {} }: Pick<StoreQueryOptions, 'search' | 'filters'>) => {
  let query = sb
    .from('products')
    .select('id, name, price, main_image_url, average_rating, review_count, brand');
...
export async function fetchStoreProducts(options: StoreQueryOptions) {
  ...
  const dataQuery = buildStoreQuery(options)
...
export async function fetchStoreProductById(id: string): Promise<ProductPageData | null> {
  const { data: product, error } = await sb
    .from('products')
    .select(`
      id, name, description, price, compare_at_price, main_image_url, average_rating, review_count, brand,
  ...
  if (categoryId) {
    const { data: similarData } = await sb
      .from('products')
      .select('id, name, price, main_image_url, average_rating')
