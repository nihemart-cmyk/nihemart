export {default as profilePlaceholder } from './imgs/profile-placeholder.png'
export  {default as slideImg1} from './imgs/landingpage/slide.png'
export  {default as carousel1} from './carousel-imgs/1.png'
export  {default as carousel2} from './carousel-imgs/2.png'
export  {default as carousel3} from './carousel-imgs/3.png'
export  {default as carousel4} from './carousel-imgs/4.png'
export  {default as carousel5} from './carousel-imgs/5.jpg'


// Icons
export { default as momoIcon} from './icons/mtn.png'