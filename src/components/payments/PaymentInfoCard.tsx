      ...
      <Card className="border-orange-200">
        ...
        <CardContent className="p-4 sm:p-6">
          <div className="flex items-center justify-center py-4">
            ...
            <span className="text-sm">Loading payment information...</span>
          </div>
        </CardContent>
      </Card>
    );
        ...
        <div className="border rounded-lg p-4 bg-gray-50">
            ...
            <div className="flex items-center space-x-2">
              <Badge className={getStatusColor(latestPayment.status)}>
                <span className="flex items-center space-x-1">
                  ...
                  <span>{latestPayment.status.charAt(0).toUpperCase() + latestPayment.status.slice(1)}</span>
                </span>
              </Badge>
            </div>
          </div>
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <span className="font-semibold">
                {latestPayment.amount.toLocaleString()} {latestPayment.currency}
              </span>
            </div>
            ...
            {latestPayment.kpay_transaction_id && (
              <div className="flex justify-between">
                ...
                <span className="font-mono text-xs">{latestPayment.kpay_transaction_id}</span>
              </div>
            )}

            <div className="flex justify-between">
              <span className="text-gray-600">Date</span>
              <span>{new Date(latestPayment.created_at).toLocaleDateString()}</span>
            </div>

            {latestPayment.failure_reason && (
              <div className="pt-2 border-t">
                <span className="text-red-600 text-xs">
                  <strong>Failure Reason:</strong> {latestPayment.failure_reason}
                </span>
              </div>
            )}
          </div>
          ...
          <div className="flex gap-2 mt-3">
            ...
            {latestPayment.status === 'pending' && (
              <Button
                size="sm"
                onClick={() => window.open(`/payment/${latestPayment.id}?orderId=${orderId}`, '_blank')}
                className="text-xs"
              >
                View Payment
              </Button>
          ...
          <div>
            ...
            <div className="space-y-2">
              {payments.slice(1).map((payment) => (
                <div key={payment.id} className="border rounded p-3 bg-white">
                  <div className="flex items-center justify-between">
