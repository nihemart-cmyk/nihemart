...
const TopBar: FC<TopBarProps> = (props) => {
  ...
  return <div className={cn('w-full py-3 px-3 lg:px-10 flex items-center justify-between border-b border-b-brand-border bg-white', { "bg-surface-secondary": variant === 'secondary' }, className)}>
    <Sheet>
      <SheetTrigger asChild>
        <Button variant={'ghost'} className="lg:hidden px-0 mr-4">
          <Menu />
        </Button>
      </SheetTrigger>
      <SheetContent side={'left'} className='lg:hidden pt-3 pb-6 pr-6 pl-0'>
        <SheetTitle className='sr-only'>Edit profile</SheetTitle>
        <Sidebar />
      </SheetContent>
    </Sheet>
    {variant === 'primary' && <h3 className='font-bold text-3xl w-full hidden md:block'>{props.title}</h3>}
    <div className={cn("flex items-center justify-between gap-3 lg:gap-6", { "w-full": variant === 'secondary' })}>
