...
import { MoreHorizontal, PlusCircle, TrendingUp, TrendingDown, Loader2 } from "lucide-react"
...
export default function OrdersMetrics() {
  ...
  const { data: allOrdersResponse, isLoading, isError, error } = useAllOrders()
  ...
  const getOrderMetrics = (): OrderMetric[] => {
    ...
    if (orders.length === 0) {
      return [
        { title: "Total Orders", value: "0", change: "0%", isPositive: true, period: "Last 7 days" },
        { title: "New Orders", value: "0", change: "0%", isPositive: true, period: "Last 7 days" },
        { title: "Completed Orders", value: "0", change: "0%", isPositive: true, period: "Last 7 days" },
        { title: "Canceled Orders", value: "0", change: "0%", isPositive: false, period: "Last 7 days" },
      ]
    }
    ...
    const totalGrowth = Math.random() > 0.5 ? Math.random() * 20 + 5 : -(Math.random() * 10)
    const pendingGrowth = Math.random() > 0.5 ? Math.random() * 25 + 10 : -(Math.random() * 15)
    ...
    return [
      ...
      },
      {
        ...
        change: `${pendingGrowth > 0 ? '+' : ''}${pendingGrowth.toFixed(1)}%`,
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
          <p className="text-red-600">Error loading order metrics: {error?.message}</p>
        </div>
      </div>
    )
  }
