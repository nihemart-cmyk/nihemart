...
const ProductCard = ({ product }: { product: StoreProduct }) => (
    <Link href={`/products/${product.id}`} className='shrink-0 group aspect-[9/12] bg-gray-100 rounded-2xl overflow-hidden relative'>
...
const ProductGridSkeleton = ({ count = 4 }: { count?: number }) => (
    <div className="grid grid-cols-1 min-[500px]:grid-cols-2 min-[1000px]:grid-cols-3 xl:grid-cols-4 gap-5">
        {Array.from({ length: count }).map((_, index) => (
            <div key={index} className='shrink-0 aspect-[9/12] bg-gray-200 rounded-2xl animate-pulse' />
        ))}
    </div>
);
...
const MoreProducts: FC<MoreProductsProps> = ({ }) => {
    ...
    const categories = useMemo(() => {
        ...
        allFeatured.forEach(product => {
            if (product.category) {
                categoryMap.set(product.category.id, { id: product.category.id, name: product.category.name });
            }
        });
    ...
    const filteredFeaturedProducts = useMemo(() => {
        ...
        return allFeatured.filter(product => product.category?.id === selectedCategoryId);
    }, [selectedCategoryId, allFeatured]);

    return <div>
        <MaxWidthWrapper size={'lg'} className=''>
            <h3 className='text-4xl font-bold text-neutral-900 mb-5'>Featured products</h3>
            <div className="flex items-center flex-wrap gap-3 mb-8">
                <Button size={ButtonSize} className='rounded-full' variant={selectedCategoryId === 'all' ? "default" : "secondary"} onClick={() => setSelectedCategoryId('all')}>All</Button>
            ...
            {loading ? <ProductGridSkeleton count={12} /> :
                <div className="grid grid-cols-1 min-[500px]:grid-cols-2 min-[1000px]:grid-cols-3 xl:grid-cols-4 gap-5">
                    {filteredFeaturedProducts.length > 0 ? (
                        ...
                        <p className="col-span-full text-center">No featured products found in this category.</p>
                    )}
                </div>
            }
        ...
        <MarqueeBanner />
        <MaxWidthWrapper size={'lg'} className="grid grid-cols-2 md:grid-cols-4 gap-5 my-20">
            {services.map(({ label, Icon }, i) => <div key={i} className='flex flex-col items-center text-center gap-2'>
                <Icon />
