...
export const BulkUploadModal: React.FC<BulkUploadModalProps> = ({ isOpen, onClose, onUploadComplete, categories, subcategories }) => {
  ...
  const categoryMap = useMemo(() => new Map(categories.map(c => [c.name.toLowerCase(), c.id])), [categories]);
  const subcategoryMap = useMemo(() => new Map(subcategories.map(s => [s.name.toLowerCase(), { id: s.id, category_id: s.category_id }])), [subcategories]);
    ...
    reader.onload = (e) => {
      try {
        ...
        if (missingHeaders.length > 0) {
          setGeneralErrors([`Missing required column headers: ${missingHeaders.join(', ')}.`]);
          setIsProcessing(false);
          return;
        }
        ...
        const processedRows: ParsedProductRow[] = json.map((row, index) => {
          ...
          if (!row.name || typeof row.name !== 'string' || row.name.trim().length === 0) errors.name = "Name is required.";
          if (row.price === undefined || isNaN(parseFloat(row.price))) errors.price = "Price must be a number.";
          if (row.stock === undefined || !Number.isInteger(Number(row.stock))) errors.stock = "Stock must be a whole number.";
          
          const categoryName = row.category_name?.toLowerCase();
          if (!categoryName || !categoryMap.has(categoryName)) errors.category_name = `Invalid category: "${row.category_name}".`;
          
          const subcategoryName = row.subcategory_name?.toLowerCase();
          if (!subcategoryName || !subcategoryMap.has(subcategoryName)) errors.subcategory_name = `Invalid subcategory: "${row.subcategory_name}".`;
        ...
        setGeneralErrors(['Failed to process the file. Please ensure it is a valid Excel file.']);
      } finally {
        setIsProcessing(false);
      }
    };
    ...
    try {
      const productsToCreate = parsedRows.map(row => {
        ...
        return {
          ...
          subcategory_id: subcategoryMap.get(subcategory_name!.toLowerCase())?.id,
        };
      });
      ...
      <DialogContent className="max-w-6xl h-[90vh] flex flex-col">
        ...
        <div className="flex-grow grid grid-cols-1 md:grid-cols-4 gap-6 overflow-hidden">
          <div className="col-span-1 flex flex-col gap-4">
            <div {...getRootProps()} className="flex-grow border-2 border-dashed rounded-lg flex flex-col items-center justify-center text-center cursor-pointer hover:border-primary p-4">
              <input {...getInputProps()} />
              <UploadCloud className="w-10 h-10 text-gray-400 mb-2" />
              {isDragActive ? <p>Drop file here</p> : <p>Drag & drop or click to upload</p>}
            </div>
            {file && <div className="p-3 border rounded-lg text-sm"><p className="font-medium truncate">{file.name}</p></div>}
            ...
            {parsedRows.length > 0 && !isProcessing && (
              <>
                <div className='flex justify-between items-center'>
                  ...
                  {hasErrors && <p className='text-sm text-red-600'>Please fix the errors highlighted in red.</p>}
                </div>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {parsedRows.map((row, index) => {
                         const categoryId = categoryMap.get(row.data.category_name?.toLowerCase() ?? '');
                         const validSubcategories = subcategories.filter(s => s.category_id === categoryId);

                         return (
                          <TableRow key={row.rowIndex} className={cn(Object.keys(row.errors).length > 0 && 'bg-red-50')}>
                            <TableCell>{row.rowIndex}</TableCell>
                            <TableCell>{row.data.name}</TableCell>
                            <TableCell>{row.data.price}</TableCell>
                            <TableCell>{row.data.stock}</TableCell>
                            <TableCell>
                              <Select value={row.data.category_name} onValueChange={(value) => handleRowChange(index, 'category_name', value)}>
                                <SelectTrigger className={cn(row.errors.category_name && 'border-red-500')}><SelectValue placeholder="Select category..." /></SelectTrigger>
                                <SelectContent>
                            ...
                            <TableCell>
                               <Select value={row.data.subcategory_name} onValueChange={(value) => handleRowChange(index, 'subcategory_name', value)} disabled={!categoryId}>
                                <SelectTrigger className={cn(row.errors.subcategory_name && 'border-red-500')}><SelectValue placeholder="Select subcategory..." /></SelectTrigger>
                                <SelectContent>
                                  {validSubcategories.map(sub => <SelectItem key={sub.id} value={sub.name}>{sub.name}</SelectItem>)}
                                </SelectContent>
                              </Select>
                            </TableCell>
                          </TableRow>
        ...
        <DialogFooter>
          <DialogClose asChild><Button variant="outline" onClick={resetState}>Cancel</Button></DialogClose>
          <Button onClick={handleSubmit} disabled={parsedRows.length === 0 || isSubmitting || isProcessing || hasErrors}>
