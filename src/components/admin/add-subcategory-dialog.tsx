...
import { createSubcategory, updateSubcategory } from '@/integrations/supabase/subcategories';
...
const subcategorySchema = z.object({
  name: z.string().min(2, 'Subcategory name must be at least 2 characters long.'),
...
export default function AddEditSubcategoryDialog({
  ...
  const onSubmit = async (values: SubcategoryFormData) => {
    try {
      const subcategoryData = { name: values.name, category_id: values.category_id };
      if (isEditMode) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>{isEditMode ? 'Edit Subcategory' : 'Add New Subcategory'}</DialogTitle>
        </DialogHeader>
        <Form {...form}>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input placeholder="e.g., Smartphones" {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )}
            />
